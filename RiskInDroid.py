...
class RiskInDroid(object):
    ...
    def get_training_vectors2(self):
        """
        replace get_training_vectors()
        """
        from pymongo import MongoClient
        client = MongoClient('localhost', 27017)
        db = client['Challenge']
        cursor = db.bPermissions.find({},{"allTypes":1,"malicious":1,"_id":0})
        _apks = []
        _targets = []
        for document in cursor:
            _apks.append(document['allTypes'])
            _targets.append(document['malicious'])
        #np_apks = np.array(_apks)
        #np_targets = np.array(_targets)
        client.close()
        return _apks, _targets
    def train_classifiers(self):
        ...
        _vectors, _targets = self.get_training_vectors2()
        self.trained_models = []
    ...
    def calculate_set_accuracy(self):
        ...
        _apks, _targets = self.get_training_vectors2()
